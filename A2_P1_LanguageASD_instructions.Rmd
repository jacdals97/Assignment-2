...
library(pacman)
p_load(tidyverse, ggplot2, pastecs, reshape, lme4)
p_load(numDeriv)
...
clean %>% subset(VISIT == 1) %>% group_by(Diagnosis, Gender) %>% summarise(Agem = mean(Age), n()) #diagnosis and gender
clean %>% subset(VISIT == 1) %>% group_by(Diagnosis, Ethnicity) %>% summarise(n())
clean %>% subset(VISIT == 1) %>% group_by(Diagnosis) %>% summarise(ADOSm = mean(ADOSv1))
...
lmerControl
model2 <- lmer(CHI_MLU ~ VISIT*Diagnosis + (1+VISIT|SUBJ), clean, REML = FALSE, control = lmerControl())
model1 <- lmer(CHI_MLU ~ VISIT * Diagnosis + (1 | SUBJ) + (0 + VISIT | SUBJ), clean)

model1_1 <- lmer(CHI_MLU ~ VISIT * Diagnosis + (1 + VISIT | SUBJ), clean, REML = T)


model0 <- lmer(CHI_MLU ~ 1 + (1+VISIT|SUBJ), clean, REML = FALSE, control = lmerControl())
model0_1 <- lmer(CHI_MLU ~ 1 + (1 | SUBJ) + (0 + VISIT | SUBJ), clean)
...
```{r}
#I don't know the math but others do

tt <- getME(model1_1,"theta")
ll <- getME(model1_1,"lower")
min(tt[ll==0]) #output should be more than 10^-6

#using package numDeriv to calculate gradients
derivs1 <- model1_1@optinfo$derivs
sc_grad1 <- with(derivs1,solve(Hessian,gradient))
max(abs(sc_grad1))

#restarting from last fit
ss <- getME(model1_1,c("theta","fixef"))
m2 <- update(model1_1,start=ss,control=lmerControl(optCtrl=list(maxfun=2e4)))
